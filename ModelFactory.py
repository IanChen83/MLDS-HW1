...
class ModelFactory:
    def __init__(self, _i_dim, _o_dim, _layer_neuron_num_list=None, _num_batch=1, _lr=0.5):
        ...
        self.x_input = tensor.fmatrix(name="X_input")  # ,dtype = theano.config.floatX)
        self.y_input = tensor.fmatrix(name="Y_input")  # ,dtype = theano.config.floatX)
        self.W_array = []
        self.B_array = []
        self.vW_array = []
        self.vB_array = []
    ...
    def _define_update_function(self):
        ...
        self.cost_function = function([self.x_input, self.y_input], self.cost, allow_input_downcast=True,
    ...
    def _cost_function(func, out):
        # e=tensor.eye(48,48,0)
        # return -(tensor.log(tensor.dot(func.T,out))).trace()
        return (abs(func - out) ** 2).sum()
        # return  tensor.mean((func-out).norm(2,axis=1))
    ...
    def load_parm(self, w_input, b_input):
        temp = [self.input_dim] + self.layer_neuron_num_list + [self.output_dim]
        for i in range(len(temp) - 1):
